...
def partition_cost(
    ...
    Raises:
        ValueError: If the partition does not cover every graph node.
    """
    ...
    uncovered_nodes = set(graph.nodes) - group_assignment.keys()
    if uncovered_nodes:
        raise ValueError(
            "Partition does not cover every graph node; missing "
            f"{sorted(uncovered_nodes)}."
        )
