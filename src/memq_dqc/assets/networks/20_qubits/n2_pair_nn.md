...
20-qubit circuit partitioned across 2 QPUs in a **pair** arrangement, **nearest neighbour** intra-QPU connectivity.
...
| Circuit qubits | 20 |
| QPUs | 2 |
| Data qubits per QPU | 10 (= ceil(20 / 2)) |
| Total data qubits | 20 |
| Spare data qubits | 0 |
...
Data qubits sit on a 4 x 3 row-major grid with 4-way (up/down/left/right) adjacency. Each communication qubit hangs off a single data qubit on the grid boundary; the attachment points are spread evenly around the perimeter.
...
* 0 spare data qubits beyond the 20 circuit qubits, because the per-QPU count is rounded up so all QPUs are identical.
