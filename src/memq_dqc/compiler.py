...
from memq_dqc.preprocessing.qasm.io import dump_qasm_program
    ...
    def to_scheduling_instance(
        ...
        *,
        ...
        return build_scheduling_instance(
            ...
            self.network,
            options=options,
            compiler_version=_memq_dqc_version(),
...
def _fingerprint_from_parts(
    ...
    """
    from memq_dqc.scheduler.instance import SCHEDULING_INSTANCE_SCHEMA_VERSION
...
def _network_signature(network: NetworkGraph) -> dict[str, Any]:
    ...
    from memq_dqc.scheduler.schedule import _physical_qubit_label
...
def _memq_dqc_version() -> str:
    """Return the installed memq_dqc version, or a sentinel when unknown."""
    try:
        return version("memq_dqc")
